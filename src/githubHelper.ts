...
export class GithubHelper {
  ...
  async processNote(
    ...
    await this.getUserGitHubApi(note.author).issues
      .createComment({
        owner: this.githubOwner,
        repo: this.githubRepo,
  ...
  async createPullRequest(mergeRequest: GitLabMergeRequest) {
    ...
    let bodyConverted = await this.convertIssuesAndComments(
      ...
      mergeRequest,
      !this.userTokenAvailable(mergeRequest.author) // || !settings.useIssueImportAPI
    );
  ...
  async updatePullRequestData(
    ...
    return await this.getUserGitHubApi(mergeRequest.author).issues.update(props);
  }
  ...
  async updatePullRequestState(
    ...
    return await this.getDefaultGitHubApi().issues.update(props);
  }
